...
- **Add-wish form state** — `use-add-wish.ts` intentionally owns only the mutation (cache, optimistic updates, retry); field values, blur/submitted UX, and when to show errors stay in `create-wish-form.tsx` with rules in `validate.ts`. As features grow, a dedicated `useAddWishFormFields` hook could centralize that controller logic if a second add-wish UI appears. Component-local `useState` is fine for now.
